...
songHistory.sort(function (a, b) { return new Date(b.ts).getTime() - new Date(a.ts).getTime(); });
...
// sort songs by play count (desc)
let songPlayCounts = Object.values(songMap);
songPlayCounts.sort(function (a, b) { return b.song_playcount - a.song_playcount; });
...
// sort artists by total play count (desc)
let artistSongPlayCounts = Object.values(artistMap);
artistSongPlayCounts.sort(function (a, b) { return b.artist_playcount - a.artist_playcount; });

// convert list of songs to the number of songs
let artistPlayCounts = artistSongPlayCounts.map(({artist, artist_playcount, songs}) => ({artist, artist_playcount, num_songs: songs.length}));

// group songs by artist play count
let artistSongFlatPlayCounts = [];
artistSongPlayCounts.forEach(artist => {
    artist.songs.forEach(song => {
        song.artist_playcount = artist.artist_playcount;
        artistSongFlatPlayCounts.push(song);
    });
});
...
writeJSON('spotify_playcount_artists_songs.json', artistSongPlayCounts);
writeJSON('spotify_playcount_artists_songs_flat.json', artistSongFlatPlayCounts);
